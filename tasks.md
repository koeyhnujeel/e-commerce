...
- [x] 토스 관련 환경변수를 `application.yml` 또는 설정 클래스에 반영한다.
- [x] 프로파일별 설정 방식을 정리한다.
- [x] 보안상 로그에 남기면 안 되는 필드를 정리한다.
- [x] 외부 API 오류와 도메인 오류 매핑 규칙을 정리한다.

### observability and docs

- [x] 주문 상태 변경 로그를 추가한다.
- [x] 결제 상태 변경 로그를 추가한다.
- [x] 토스 호출 실패 로그 마스킹 규칙을 적용한다.
- [x] 로컬/테스트베드 실행 가이드를 문서화한다.
- [x] 토스 테스트 결제 절차를 문서화한다.

### tests and architecture

- [x] `ModulithArchitectureTests`를 현재 모듈 구조에 맞게 갱신한다.
- [x] `payment` 모듈 공개 API/의존 방향을 아키텍처 테스트에 반영한다.
- [x] 전체 영향 범위를 확인하기 위한 테스트 실행 계획을 정리한다.
- [x] 주요 시나리오 회귀 테스트 목록을 만든다.
