...
- [ ] `Product`에 판매 상태 필드 또는 판매 가능 여부 판단 메서드를 추가한다.
- [ ] `Product`에 생성일시/수정일시가 필요한지 결정한다.
- [ ] 대표 이미지 선택 로직의 위치를 정한다.
- [ ] 옵션의 주문 가능 여부 판단 기준을 정한다.
...
- [ ] 상품 상세 조회 DTO를 정의한다.
- [ ] 상품 목록 정렬 enum을 정의한다.
- [ ] 상품 목록 조회 서비스 유스케이스를 추가한다.
- [ ] 상품 상세 조회 서비스 유스케이스를 추가한다.

### catalog persistence

- [ ] 상품 목록 조회용 repository 쿼리를 추가한다.
- [ ] 카테고리 필터 조건을 지원한다.
- [ ] 가격 정렬 조건을 지원한다.
- [ ] 최신순 정렬 조건을 지원한다.
- [ ] 상품 상세 조회 시 이미지/옵션 fetch 전략을 조정한다.

### catalog adapter/in

- [ ] `GET /api/products` 컨트롤러를 추가한다.
- [ ] `GET /api/products/{productId}` 컨트롤러를 추가한다.
- [ ] 페이지네이션 쿼리 파라미터를 요청 객체로 정리할지 결정한다.

### tests

- [ ] 상품 판매 상태 도메인 테스트를 추가한다.
- [ ] 상품 목록 서비스 테스트를 추가한다.
- [ ] 상품 상세 서비스 테스트를 추가한다.
- [ ] 상품 목록 API 통합 테스트를 추가한다.
- [ ] 상품 상세 API 통합 테스트를 추가한다.

## Phase 2. 좋아요 읽기 연동

### like module

- [ ] 상품별 좋아요 수 조회 포트를 추가한다.
- [ ] 회원 기준 상품 좋아요 여부 조회 포트를 추가한다.
- [ ] 상품 ID 목록 기준 벌크 좋아요 집계 조회를 추가한다.
- [ ] `LikeRepository` 또는 adapter 쿼리를 확장한다.

### catalog integration

- [ ] 상품 목록 응답에 좋아요 수를 포함한다.
- [ ] 상품 상세 응답에 좋아요 수를 포함한다.
- [ ] 상품 목록 응답에 `likedByMe`를 포함한다.
- [ ] 상품 상세 응답에 `likedByMe`를 포함한다.
- [ ] 비로그인 사용자의 `likedByMe = false` 처리 로직을 추가한다.

### tests

- [ ] 좋아요 집계 조회 repository 테스트를 추가한다.
- [ ] 상품 목록 응답 조합 서비스 테스트를 추가한다.
- [ ] 상품 상세 응답 조합 서비스 테스트를 추가한다.
- [ ] 로그인 사용자 상품 목록 API 테스트를 추가한다.
- [ ] 비로그인 사용자 상품 목록 API 테스트를 추가한다.
- [ ] 로그인 사용자 상품 상세 API 테스트를 추가한다.

## Phase 3. 주문 생성, 목록, 상세

### order domain

- [ ] `Order`에 `orderNumber` 필드를 추가한다.
- [ ] `Order`에 `totalAmount` 필드를 추가한다.
- [ ] `Order` 생성 규칙을 재설계한다.
- [ ] `OrderItem`에 상품명 스냅샷 필드를 추가한다.
- [ ] `OrderItem`에 옵션 색상/사이즈 스냅샷 필드를 추가한다.
- [ ] `OrderItem`에 주문 당시 가격 스냅샷 정책을 정리한다.
- [ ] 주문 총액 계산 책임을 도메인에 추가한다.

### order application

- [ ] 주문 생성 command/DTO를 정의한다.
- [ ] 주문 목록 조회 DTO를 정의한다.
- [ ] 주문 상세 조회 DTO를 정의한다.
- [ ] 주문 생성 서비스 유스케이스를 추가한다.
- [ ] 내 주문 목록 조회 서비스 유스케이스를 추가한다.
- [ ] 내 주문 상세 조회 서비스 유스케이스를 추가한다.
- [ ] 타인 주문 접근 차단 로직을 추가한다.

### cross-module integration

- [ ] `catalog`에 주문 생성용 상품/옵션 검증 API를 보강한다.
- [ ] 주문 생성 시 상품 존재 여부를 검증한다.
- [ ] 주문 생성 시 옵션 존재 여부를 검증한다.
- [ ] 주문 생성 시 서버 계산 금액만 사용하도록 처리한다.

### order persistence

- [ ] 주문 저장 구조 변경에 맞게 JPA 매핑을 수정한다.
- [ ] 내 주문 목록 조회 쿼리를 추가한다.
- [ ] 내 주문 상세 조회 쿼리를 추가한다.
- [ ] 주문 번호 유니크 제약을 추가한다.
...
- [ ] `POST /api/orders` 컨트롤러를 추가한다.
- [ ] `GET /api/orders` 컨트롤러를 추가한다.
- [ ] `GET /api/orders/{orderId}` 컨트롤러를 추가한다.
- [ ] 주문 생성/조회 엔드포인트를 인증 대상으로 추가한다.

### tests

- [ ] 주문 생성 도메인 테스트를 추가한다.
- [ ] 주문 총액 계산 도메인 테스트를 추가한다.
- [ ] 주문 생성 서비스 테스트를 추가한다.
- [ ] 내 주문 목록 서비스 테스트를 추가한다.
- [ ] 내 주문 상세 서비스 테스트를 추가한다.
- [ ] 주문 생성 API 통합 테스트를 추가한다.
- [ ] 내 주문 목록 API 통합 테스트를 추가한다.
- [ ] 내 주문 상세 API 통합 테스트를 추가한다.

## Phase 4. 주문 수정, 삭제, 상태 전이 정리

### order domain

- [ ] `OrderStatus`를 `CREATED`, `PAYMENT_PENDING`, `PAID`, `CANCELED`, `DELIVERED` 기준으로 정리한다.
- [ ] `OrderItemStatus`를 주문 상태와 정합성 있게 정리한다.
- [ ] 주문 수정 가능 여부 판단 메서드를 추가한다.
- [ ] 주문 삭제 가능 여부 판단 메서드를 추가한다.
- [ ] 주문 취소/삭제 상태 전이 메서드를 추가한다.

### order application

- [ ] 주문 수정 command/DTO를 정의한다.
- [ ] 주문 수정 서비스 유스케이스를 추가한다.
- [ ] 주문 삭제 서비스 유스케이스를 추가한다.
- [ ] 결제 완료 주문 수정 차단 로직을 추가한다.
- [ ] 결제 완료 주문 삭제 차단 로직을 추가한다.

### order persistence

- [ ] 소프트 삭제를 사용할지 결정한다.
- [ ] 소프트 삭제를 사용하면 `deletedAt` 또는 `isDeleted` 필드를 추가한다.
- [ ] 주문 수정 시 전체 주문상품 재구성 저장 로직을 검증한다.

### order adapter/in

- [ ] `PATCH /api/orders/{orderId}` 컨트롤러를 추가한다.
- [ ] `DELETE /api/orders/{orderId}` 컨트롤러를 추가한다.

### tests

- [ ] 주문 수정 도메인 테스트를 추가한다.
- [ ] 주문 삭제 도메인 테스트를 추가한다.
- [ ] 주문 상태 전이 도메인 테스트를 추가한다.
- [ ] 주문 수정 서비스 테스트를 추가한다.
- [ ] 주문 삭제 서비스 테스트를 추가한다.
- [ ] 주문 수정 API 통합 테스트를 추가한다.
- [ ] 주문 삭제 API 통합 테스트를 추가한다.
